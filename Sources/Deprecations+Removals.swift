...
extension Signal {
	@available(*, unavailable, renamed:"flatMap(_:_:)")
	public func flatMap<Inner: ExpressibleBySignalProducer>(_ strategy: FlattenStrategy, transform: @escaping (Value) -> Inner) -> Signal<Inner.Value, Error> where Error == Inner.Error { fatalError() }

	@available(*, unavailable, renamed:"flatMap(_:_:)")
	public func flatMap<Inner: ExpressibleBySignalProducer>(_ strategy: FlattenStrategy, transform: @escaping (Value) -> Inner) -> Signal<Inner.Value, Error> where Inner.Error == NoError { fatalError() }
}

extension Signal where Error == NoError {
	@available(*, unavailable, renamed:"flatMap(_:_:)")
	public func flatMap<Inner: ExpressibleBySignalProducer>(_ strategy: FlattenStrategy, transform: @escaping (Value) -> Inner) -> Signal<Inner.Value, Inner.Error> where Error == Inner.Error { fatalError() }

	@available(*, unavailable, renamed:"flatMap(_:_:)")
	public func flatMap<Inner: ExpressibleBySignalProducer>(_ strategy: FlattenStrategy, transform: @escaping (Value) -> Inner) -> Signal<Inner.Value, Inner.Error> { fatalError() }
}

extension SignalProducer {
	@available(*, unavailable, renamed:"flatMap(_:_:)")
	public func flatMap<Inner: ExpressibleBySignalProducer>(_ strategy: FlattenStrategy, transform: @escaping (Value) -> Inner) -> SignalProducer<Inner.Value, Error> where Error == Inner.Error { fatalError() }

	@available(*, unavailable, renamed:"flatMap(_:_:)")
	public func flatMap<Inner: ExpressibleBySignalProducer>(_ strategy: FlattenStrategy, transform: @escaping (Value) -> Inner) -> SignalProducer<Inner.Value, Error> where Inner.Error == NoError { fatalError() }
}

extension SignalProducer where Error == NoError {
	@available(*, unavailable, renamed:"flatMap(_:_:)")
	public func flatMap<Inner: ExpressibleBySignalProducer>(_ strategy: FlattenStrategy, transform: @escaping (Value) -> Inner) -> SignalProducer<Inner.Value, Inner.Error> where Error == Inner.Error { fatalError() }

	@available(*, unavailable, renamed:"flatMap(_:_:)")
	public func flatMap<Inner: ExpressibleBySignalProducer>(_ strategy: FlattenStrategy, transform: @escaping (Value) -> Inner) -> SignalProducer<Inner.Value, Inner.Error> { fatalError() }
}
...
@available(*, unavailable, message: "This protocol has been removed. Constrain `Signal` directly instead.")
public protocol SignalProtocol {}

@available(*, unavailable, message: "This protocol has been removed. Constrain `SignalProducer` directly instead.")
